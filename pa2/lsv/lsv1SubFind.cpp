...
{  
 ...
 double time1=(double)clock() / CLOCKS_PER_SEC;
  ...
  //Abc_Ntk_t * pMiter,*pNtk2, *pNtk3,*pNtkon, *pNtkoff, *pNtk1;
  Abc_Obj_t * pObj, *pNode;

  selectCandidateforpi(pNtk);   
  Abc_NtkForEachPi(pNtk, pNode, id){   
    minisatPtr=new SatSolver();
    initSolver( minisatPtr,pNtk);
    addCircuitCNF( minisatPtr,pNtk,0,-1);
    addCircuitCNF( minisatPtr,pNtk,1,-1);
   
    buildMiter( pNtk,id);
   // buildActivateVar(pNtk,id);
   // check(pNtk,id);
    delete minisatPtr;
  }
  printf("\n");
  ...
  selectCandidate(pNtk);
   
  Abc_NtkForEachNode(pNtk, pNode, id){   
    minisatPtr=new SatSolver();
    initSolver( minisatPtr,pNtk);
    addCircuitCNF( minisatPtr,pNtk,0,id);
    addCircuitCNF( minisatPtr,pNtk,1,id);
    num=id- Abc_NtkPiNum(  pNtk )-Abc_NtkPoNum(  pNtk )-1;

    buildMiter( pNtk,num);
    buildActivateVar(pNtk,num);
    check(pNtk,num);
    delete minisatPtr;
  }
  printf("\n");
...
void
Lsv_Ntk1MAJFind( Abc_Ntk_t * pNtk )
{
   // TODO
  int count=0;
  Abc_Obj_t * pObj;
